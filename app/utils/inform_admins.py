...
async def inform_admins(message, from_id, album_builder = None):
    if isinstance(config_aiogram.admin_id, list):
        for a in config_aiogram.admin_id:
            try:
                if album_builder:
                    ...
                    await aiogram_bot.send_message(chat_id=a, text=message, reply_markup=main_kb.admin_confirmation(from_id))
                else:
                    await aiogram_bot.send_message(chat_id=a, text=message, reply_markup=main_kb.admin_confirmation(from_id))
        ...
        try:
            if album_builder:
                ...
                await aiogram_bot.send_message(chat_id=config_aiogram.admin_id, text=message, reply_markup=main_kb.admin_confirmation(from_id))
            else:
                await aiogram_bot.send_message(chat_id=config_aiogram.admin_id, text=message, reply_markup=main_kb.admin_confirmation(from_id))
        except Exception as e:
            logger.error(e)
